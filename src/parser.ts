 ...
 * FIELDS    := head=FIELD _ tail={ _ ',' _ value=FIELD}*
 *              .value = FIELD[] { return [this.head].concat(tail.map(e => e.value)); }
...
export class FIELDS {
  ...
  public head: FIELD;
  public tail: FIELDS_$0[];
  public value: FIELD[];
  constructor(head: FIELD, tail: FIELDS_$0[]) {
    this.head = head;
    this.tail = tail;
    this.value = ((): FIELD[] => {
      return [this.head].concat(tail.map((e) => e.value));
    })();
  }
}
...
export class Parser {
  ...
  public matchPROGRAM($$dpth: number, $$cr?: ErrorTracker): Nullable<PROGRAM> {
    return this.run<PROGRAM>($$dpth, () => {
      ...
      if (
        true &&
        ($scope$toplevels = this.loop<TOPLEVEL>(
          () => this.matchTOPLEVEL($$dpth + 1, $$cr),
          0,
          -1,
        )) !== null &&
  ...
  public matchSTRUCTDEF(
    ...
    return this.run<STRUCTDEF>($$dpth, () => {
      ...
      if (
        ...
        this.regexAccept(String.raw`(?:struct)`, "", $$dpth + 1, $$cr) !==
          null &&
  ...
  public matchFIELDS($$dpth: number, $$cr?: ErrorTracker): Nullable<FIELDS> {
    return this.run<FIELDS>($$dpth, () => {
      let $scope$head: Nullable<FIELD>;
      let $scope$tail: Nullable<FIELDS_$0[]>;
      let $$res: Nullable<FIELDS> = null;
      if (
        true &&
        ($scope$head = this.matchFIELD($$dpth + 1, $$cr)) !== null &&
        this.match_($$dpth + 1, $$cr) !== null &&
        ($scope$tail = this.loop<FIELDS_$0>(
          () => this.matchFIELDS_$0($$dpth + 1, $$cr),
          0,
          -1,
        )) !== null
      ) {
        $$res = new FIELDS($scope$head, $scope$tail);
      }
      return $$res;
    });
  }
  public matchFIELDS_$0(
    $$dpth: number,
    $$cr?: ErrorTracker,
  ): Nullable<FIELDS_$0> {
    return this.run<FIELDS_$0>($$dpth, () => {
      ...
      if (
        ...
        this.regexAccept(String.raw`(?:,)`, "", $$dpth + 1, $$cr) !== null &&
        this.match_($$dpth + 1, $$cr) !== null &&
        ($scope$value = this.matchFIELD($$dpth + 1, $$cr)) !== null
      ) {
        $$res = { kind: ASTKinds.FIELDS_$0, value: $scope$value };
      }
      return $$res;
    });
  }
